...
class Cart {
  ...
  // 取得 Order_Temp 所有商品
    ...
    // `;
    const sql=`SELECT ot.Order_Sid,ot.Product_id,ot.Order_Amount,pf.name,pf.price,pf.cate_sid FROM ${tableName} ot LEFT JOIN products_food pf ON ot.Product_id=pf.product_id`;
  ...
  // 加入購物車
   static async addtmpList(Order_Sid,Member_id,Product_id,Order_Amount) {
    const output = {
      success: false,
      error: "",
    };
  ...
  // 更新購物車
  static async updatetmpList(Order_Sid,Order_Amount) {
    const output = {
      success: false,
      error: "",
    };
    ...
    const [r] = await db.query(sql, [obj, Order_Sid]);
  ...
  // 刪除購物車單一品項
  static async removetmpList(Order_Sid){
    const output={
      success:false,
      error:'',
    }
